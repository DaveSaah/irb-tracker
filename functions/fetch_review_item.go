...
func FetchReviewItem(userID, projID int) (model.Project, error) {
	conn, err := db.Init()
	if err != nil {
    ...
    results_dissemination, status.sname, date_submitted
    FROM projects
    ...
    INNER JOIN users
    ON projects.principal_investigator = users.id
    WHERE projects.supervisor= ?
    AND projects.id = ?;
    `, userID, projID).Scan(
		&project.ID,
		&project.Title,
